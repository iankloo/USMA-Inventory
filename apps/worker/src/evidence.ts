...
function validateEvidence(evidence: AuditEvidence): AuditEvidenceItem[] {
  if (!evidence.auditId.trim()) throw new Error("Audit evidence requires an audit ID.");
  if (!evidence.physicalCountFinalizedAt) throw new Error("Only physically finalized audits can be exported.");
  const snapshot = new Set(evidence.snapshotSerials.map(normalizeSerial));
  if (snapshot.size !== evidence.snapshotSerials.length) throw new Error("Snapshot contains duplicate or invalid serials.");
  ...
  if (resolved.size !== items.length) throw new Error("Audit evidence contains duplicate resolution items.");
  const unresolved = [...snapshot].filter((serial) => !resolved.has(serial));
  if (unresolved.length) throw new Error(`Audit evidence has unresolved snapshot serials: ${unresolved.join(", ")}.`);
  const unexpected = [...resolved].filter((serial) => !snapshot.has(serial));
  if (unexpected.length) throw new Error(`Audit evidence contains serials outside the snapshot: ${unexpected.join(", ")}.`);
  return items.sort((a, b) => a.serial.localeCompare(b.serial));
}

export function generateEvidenceCsv(evidence: AuditEvidence): string {
  ...
  for (const item of items) rows.push(line(["audit-item", item.serial, item.resolution, item.resolvedAt, item.actor, item.reason ?? "", item.eventReferences?.join(" ") ?? ""]));
...
export function generateEvidencePackage(evidence: AuditEvidence): { csv: string; pdf: Uint8Array } {
