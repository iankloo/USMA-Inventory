...
function safeFromLocation(value) {
    const match = String(value ?? "").trim().match(/^safe\s*(\d+)$/i);
    return match ? Number(match[1]) : undefined;
}
function isOffsiteLocation(value) {
...
function rowFromRecord(record, rowNumber) {
    ...
    if (safe === "invalid")
        issues.push({ row: rowNumber, field: "safe", code: "invalid-number", message: "Safe must be an integer." });
    if (slot === "invalid")
        issues.push({ row: rowNumber, field: "slot", code: "invalid-number", message: "Slot must be an integer." });
    if (typeof safe === "number" && (safe < 2 || safe > 7))
        issues.push({ row: rowNumber, field: "safe", code: "safe-out-of-range", message: "Safe must be between 2 and 7." });
    if (typeof slot === "number" && (slot < 1 || slot > 28))
        issues.push({ row: rowNumber, field: "slot", code: "slot-out-of-range", message: "Slot must be between 1 and 28." });
    if (safe === undefined && slot !== undefined)
        issues.push({ row: rowNumber, code: "incomplete-location", message: "Slot cannot be provided without a safe." });
    ...
    const type = legacyModelType
        ? legacyModelType === "sporting"
            ...
            : legacyModelType.startsWith("trap")
                ...
                : undefined
        : explicitType;
    if (type && !["skeet", "trap", "sporting"].includes(type))
        issues.push({ row: rowNumber, field: "type", code: "invalid-enum", message: "Type must be skeet, trap, or sporting." });
...
export function validateGunImport(records, options = {}) {
    ...
    records.forEach((record, index) => {
        if (options.skipBlankSerial && !String(record.serialNumber ?? "").trim())
            return;
        ...
        const decision = existing.has(parsed.row.serialNumber) ? "update" : "create";
...
function recordsFromTable(table) {
    if (table.length === 0)
        return { records: [], legacyLocationHeaders: false };
    const headers = table[0].map(canonicalHeader);
    const mapped = headers.map((header) => HEADER_ALIASES[header]);
    if (!mapped.includes("serialNumber"))
        throw new Error("Import must include a Serial Number column.");
...
export function validateCsvGunImport(input, options = {}) {
...
export async function validateXlsxGunImport(input, options = {}) {
...
function locationKey(safe, slot) {
 ...
 */
export function prepareLegacyInventory(records, options = {}) {
    ...
    const normalized = records.map((record) => {
        ...
        if (typeof safe !== "number" || typeof slot !== "number")
            return next;
        if (slot < 1 || slot > 28)
            return next;
        let resolvedSlot = slot;
        const originalKey = locationKey(safe, slot);
        if (owners.get(originalKey) === normalizeSerial(String(record.serialNumber ?? "")))
