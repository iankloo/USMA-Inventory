...
export function extractSerialsFromText(pages, options = {}) {
    ...
    if (pages.every((page) => page.trim() === ""))
        warnings.push("PDF contains no selectable text.");
    if (known.size === 0)
        warnings.push("No inventory snapshot was supplied; all serial-like tokens require human review.");
    if (deduped.serials.length === 0)
        warnings.push("No serials were extracted from selectable text.");
    return {
 ...
 */
export async function extractPdfSerials(input, options = {}) {
    if (input.byteLength === 0)
        throw new Error("PDF upload is empty.");
    if (input.byteLength > 25 * 1024 * 1024)
        throw new Error("PDF upload exceeds the 25 MB worker limit.");
...
async function defaultPdfTextLoader(data) {
